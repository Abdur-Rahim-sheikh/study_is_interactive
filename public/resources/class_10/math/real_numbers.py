from sympy import nsimplify, isprime
...
class RealNumbers:
    ...
    def dfs(self, graph, cur, dest, path=[], visited=set()):
        ...
        for neighbor in graph.get(cur, []):
            if neighbor not in visited:
                result = self.dfs(graph, neighbor, dest, path, visited)
                if result:
                    return result
    ...
    def categorize(self, value:float, apostrophe=False):
        ...
        paths = []
        if not number.is_rational:
            path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "অসীম অনাবৃত দশমিক")
            paths.append(path)
            
        elif apostrophe or self.is_infinite_decimal(value):
            path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "অসীম আবৃত্ত")
            paths.append(path)

        elif number.is_Integer:
            num = int(number)
            if num < 0:
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "ঋণাত্মক")
                paths.append(path)
            elif num == 0:
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "০")
                paths.append(path)
            elif num == 1:
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "১")
                paths.append(path)
            elif isprime(num):
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "মৌলিক")
                paths.append(path)
            else:
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "যৌগিক")
                paths.append(path)
        else:
            # it's finite fractional now
            num, denum = number.as_numer_denom()
            if num<denum:
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "প্রকৃত") 
                paths.append(path)
            else:
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "অপ্রকৃত")
                paths.append(path)
                path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "মিশ্র")
                paths.append(path)
            path = self.dfs(self.getGraph, "বাস্তব সংখ্যা", "সসীম")
            paths.append(path)
